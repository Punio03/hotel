...
public:
    ...
    inline friend ostream& operator<<(ostream& out, const Room &room) noexcept { out << "ID:" << room.RoomID << endl << "Capacity:" << room.Capacity << endl<< "Price for night:" << room.Price << "$" << endl << "Availability:" << room.Available; return out; }
}; // gotowe
class Address final{
public:
    string Country;
    ...
    inline friend ostream& operator<<(ostream& out, const Address &address) noexcept { out << "Country:" << address.Country << endl << "StreetName:" << address.StreetName << endl << "ZipCode:" << address.ZipCode << endl << "City:" << address.City << endl << "Telephone Number:" << address.TelephoneNumber << endl; return out; }
...
private:
    ...
    vector<Room> Rooms;
public:
    inline Hotel(string &newName, Address &newAddress, vector<Room>&newRooms) noexcept : Name(newName), HotelAddress(newAddress), Rooms(newRooms) { }
